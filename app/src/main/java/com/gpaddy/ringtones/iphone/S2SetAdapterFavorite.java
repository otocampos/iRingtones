...
public class S2SetAdapterFavorite extends ArrayAdapter {
    ...
    public S2SetAdapterFavorite(Activity context, int layoutID, ArrayList<S1ObjectMusic> arrayObjects) {
    ...
    public View getView (final int position, View convertView, ViewGroup parent) {
            ...
            public void onClick(View v) {
                ...
                if (recentIndex == position) {
                    if (arrayObjects.get(recentIndex).isStatus()) {
                        ...
                        mediaPlayer = MediaPlayer.create(context, Uri.parse(Environment.getExternalStorageDirectory().getPath() + "/i Favorite/" + name + ".mp3"));
                        mediaPlayer.start();
        ...
        buttonSet.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                final File file = new File(Environment.getExternalStorageDirectory().getPath() + "/i Favorite/", name + ".mp3");
...
//                });
    ...
    BroadcastReceiver receiverStop = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            if (mediaPlayer.isPlaying() && arrayObjects.get(recentIndex).isStatus() == true) {
                mediaPlayer.stop();
                arrayObjects.get(recentIndex).setStatus(false);
            }
        }
